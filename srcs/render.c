...
void select_image(t_game *game, size_t y, size_t x)
{
	...
	if (game->map_grid[y][x] == '1')
		if (mlx_image_to_window(game->mlx, game->img->bush, x * PIXELS, y * PIXELS) < 0)
			write_error("Error during img to window!"); 
	if (game->map_grid[y][x] == 'C')
		if (mlx_image_to_window(game->mlx, game->img->collectible, x * PIXELS + img_size / 2, y * PIXELS + img_size / 2) < 0)
			write_error("Error during img to window!"); 
	if (game->map_grid[y][x] == 'E')
		if (mlx_image_to_window(game->mlx, game->img->exit_closed, x * PIXELS, y * PIXELS) < 0)
			write_error("Error during img to window!");
	if (game->map_grid[y][x] == 'P')
		if (mlx_image_to_window(game->mlx, game->img->player, x * PIXELS, y * PIXELS) < 0)
			write_error("Error during img to window!");
	if (game->map_grid[y][x] == 'X')
		if (mlx_image_to_window(game->mlx, game->img->enemy,x * PIXELS, y * PIXELS) < 0)
			write_error("Error during img to window!");
}
